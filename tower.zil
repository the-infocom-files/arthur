...
; "file : TOWER.ZIL"
; "auth :   $Author:   DEB  $"
; "date :     $Date:   16 May 1989 21:05:06  $"
; "revs : $Revision:   1.107  $"
; "vers : 1.0"
;"---------------------------------------------------------------------------"
; "Ivory tower"
...
<ROOM RM-TOWER-ROOM
	...
	(GLOBAL LG-WOODEN-DOOR LG-WALL LG-TOWER LG-FOREST RM-LANDING)
		...
		(<VERB? ENTER EXIT>
			<COND
				(<MC-HERE? ,RM-TOWER-ROOM>
					<COND
						(<MC-FORM? ,K-FORM-OWL>
							<RT-GOTO ,RM-ABOVE-FOREST>)
						(T
							<TELL "You would fall and hurt yourself." CR>)>)
				(<MC-HERE? ,RM-MOOR ,RM-TOWN-SQUARE>
					<RT-YOU-CANT-MSG "go through">
					<RT-AUTHOR-MSG "What do you think this is, Zork?">)>)>>

<OBJECT CH-RUMPLESTILTSKIN
	(LOC GLOBAL-OBJECTS)
	(DESC "Rumplestiltskin")
	(FLAGS FL-ALIVE FL-NO-DESC FL-PERSON)
	(SYNONYM RUMPLESTILTSKIN)
	(ACTION RT-CH-RUMPLESTILTSKIN)>

<ROUTINE RT-CH-RUMPLESTILTSKIN ("OPT" (CONTEXT <>))
	 <COND (.CONTEXT <RFALSE>)
	       (ELSE
		<TELL "Wasn't he in a fairy tale?" CR>)>>
...
nation.\"" CR>
	 ...
	 <COND
	  (.C?
	   <FSET ,CH-RHYMER ,FL-LOCKED>
	   <TELL
"The old man jumps up and claps his hands with glee." CR CR

"\"You win! You win! To thee I sing." CR
" And give to you this magic ring." CR
" When unseen forces you surprise," CR
" Just rub this ring to clear your eyes.\"" CR CR

"He gives you a ring." CR>
	   <COND
...
"|\"These guesses often greatly vex.| At least please try to match my sex!\"" CR>)
	  (<EQUAL? .S 2>
	   <TELL
"|\"If Rumplestiltskin were my name,| Then this would be a wimpy game.\"" CR>)
	  (<EQUAL? .S 3>
	   <TELL
...
"|\"Uther was your dad, it's true,| But proving it is up to you.\"" CR>)
				(<MC-PRSI? ,CH-PLAYER>
				 <TELL CR
"\"You're Britain's pride, and Britain's glory," CR
 " Your deeds writ large in England's story." CR
 " And all the world will one day sing," CR
 " Of you, our Once and Future King." CR>)
				(T
					<TELL
...
<ROUTINE RT-RHYMER-MSG ()
	...
	<COND
		(<FSET? ,CH-RHYMER ,FL-BROKEN>
			<TELL
"\"I see that you have ventured back| To claim the loot which now you
lack| But first I must make truly sure| That you have learned my moniker.\"" CR>)
		(T
			<FSET ,CH-RHYMER ,FL-BROKEN>
			<TELL
"\"You come in search of gold and loot.| I have all that, and more to
boot.| To get it you must play my game.| Just say to me my secret name.\"" CR>)>>
...
<OBJECT TH-RHYME
	(LOC GENERIC-OBJECTS)
	(DESC "rhyme")
	(SYNONYM RHYME RHYMES POETRY VERSE POEM POEMS)>
...
<ROUTINE RT-LG-WOODEN-DOOR ("OPT" (CONTEXT <>))
	<COND
		...
		(<AND <VERB? KNOCK>
		      ...
		      <MC-HERE? RM-LANDING>>
			<TELL
"A voice calls out:" CR CR

"\"Come in, come in. I'm all alone." CR
" And you don't need a chaperone.\"" CR>)>>
					...
					<COND
						(<NOT <MC-FORM? ,GL-OLD-FORM>>
							<COND
								...
								(T
									<SETG GL-PICTURE-NUM ,K-PIC-BLACK>)>
