...
<TERMINALS
	(VERB 6) (NOUN 4) (ADJ 5) ADV QUANT MISCWORD (DIR 1) 
	TOBE QWORD CANDO COMMA (PARTICLE 3) (PREP 2)
				...
				<COND
					(<AND <SET WHO <FIND-A-WINNER ,HERE>>
							<OR	; "Is this right? -- DEB"
								<EQUAL? ,GL-PLAYER-FORM ,K-FORM-ARTHUR>
...
<ROUTINE XERASE (N "AUX" X)
	 <COND (<EQUAL? .N 1>
		<SET N <- <WINGET -3 ,WWIDE>
			  <WINGET -3 ,WXPOS>
			  1>>)>
	 <SET X <MOD .N ,GL-SPACE-WIDTH>>
	 <SET N </ .N ,GL-SPACE-WIDTH>>
	 <COND (.X <SET N <+ .N 1>>)>
	 <REPEAT ((L <GETB ,GL-SPACE-TABLE 0>)
		  (OX <WINGET -3 ,WXPOS>)
		  (OY <WINGET -3 ,WYPOS>))
	    <COND (<L? .N .L>
		   <PRINTT <REST ,GL-SPACE-TABLE 1> .N>
		   <CURSET .OY .OX>
		   <RETURN>)
		  (ELSE
		   <SET N <- .N .L>>
		   <PRINTT <REST ,GL-SPACE-TABLE 1> .L>)>>>

<CONSTANT GL-SPACE-TABLE <TABLE (STRING LENGTH) "                   ">>
...
<ROUTINE RT-MAP-CLICK (C "AUX" CY CX X Y)
	 ...
	 <COND
	  (<WITHIN? ,GL-TL-Y ,GL-TL-X ,GL-BR-Y ,GL-BR-X>
	   <COND ;(<AND <EQUAL? ,GL-MACHINE ,MACINTOSH>
			<NOT <MAC-II?>>>
		   <RETURN <MAC-CLICK .CY .CX>>)
		 (<OR <EQUAL? ,GL-MACHINE ,IBM ,AMIGA ,MACINTOSH ,DEBUGGING-ZIP>
		      <MC-APPLE?>>
...
"[Sorry, clicking on the compass rose is not yet implemented on this machine.]|">
	 ...
	 <COND
	  (T ;<WITHIN? ,GL-TL-Y ,GL-TL-X ,GL-BR-Y ,GL-BR-X>
	   ...
	   <SETG GL-BR-X <- <+ ,GL-TL-X ,GL-MAP-GRID-X> 1>>
	   <COND
	    (<WITHIN? 0 ;,GL-TL-Y ,GL-TL-X ,GL-BR-Y ,GL-ROSE-X ;,GL-BR-X>
	     <RETURN ,P?NE>)>
	   ...
	   <SETG GL-BR-X <- <+ ,GL-TL-X ,GL-MAP-GRID-X> 1>>
	   <COND
	    (<WITHIN? ,GL-TL-Y 0 ;,GL-TL-X ,GL-BR-Y ,GL-BR-X>
	     <RETURN ,P?WEST>)>
	   <SETG GL-TL-X <+ ,GL-TL-X ,GL-MAP-GRID-X <ZGET ,K-WIN-TBL 1>>>
	   <SETG GL-BR-Y <- <+ ,GL-TL-Y <ZGET ,K-WIN-TBL 0>> 1>>
	   <SETG GL-BR-X <- <+ ,GL-TL-X ,GL-MAP-GRID-X> 1>>
	   <COND
	    (<WITHIN? ,GL-TL-Y ,GL-TL-X ,GL-BR-Y ,GL-ROSE-X ;,GL-BR-X>
	     <RETURN ,P?EAST>)>
	   ...
	   <SETG GL-BR-X <- <+ ,GL-TL-X ,GL-MAP-GRID-X> 1>>
	   <COND
	    (<WITHIN? ,GL-TL-Y 0 ;,GL-TL-X
		      <WINGET 2 ,WHIGH> ;,GL-BR-Y ,GL-BR-X>
	     <RETURN ,P?SW>)>
	   <SETG GL-TL-X <+ ,GL-TL-X ,GL-MAP-GRID-X>>
	   <SETG GL-BR-Y <- <+ ,GL-TL-Y ,GL-MAP-GRID-Y> 1>>
	   <SETG GL-BR-X <- <+ ,GL-TL-X <ZGET ,K-WIN-TBL 1>> 1>>
	   <COND
	    (<WITHIN? ,GL-TL-Y ,GL-TL-X
		      <WINGET 2 ,WHIGH> ;,GL-BR-Y ,GL-BR-X>
	     <COND
	   ...
	   <COND
	    (<WITHIN? ,GL-TL-Y ,GL-TL-X
		      <WINGET 2 ,WHIGH> ;,GL-BR-Y ,GL-ROSE-X ;,GL-BR-X>
	     <RETURN ,P?SE>)>)>>
...
<DEFMAC MC-APPLE? ()
   '<EQUAL? ,GL-MACHINE ,APPLE-2C ,APPLE-2E ,APPLE-2GS>>
