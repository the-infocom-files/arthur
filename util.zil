...
; "file : UTIL.ZIL"
; "auth :   $Author:   DEB  $"
; "date :     $Date:   18 May 1989 13:22:46  $"
; "revs : $Revision:   1.90  $"
; "vers : 1.0"
;"---------------------------------------------------------------------------"
; "Utility routines"
...
<ROUTINE RT-AUTHOR-OFF ("AUX" WY WX Y X W AY N L PTR SP? CLR FG BG)
	<DIROUT ,D-TABLE-OFF>
	<COND (<G? ,GL-AUTHOR-SIZE 0>
	       <RT-AUTHOR-CLEAR>)>
	...
	<WINSIZE 3 .AY .WX>
	<WINATTR 3 <ORB ,A-WRAP ,A-SCRIPT> ,O-SET>     ;"Make window 3 wrap and script."
	<CLEAR 3>
	<SCREEN 3>
	<CURSET 1 1>
	<SET CLR <WINGET -3 ,WCOLOR>>
	<SET BG <LSH .CLR -8>>
	<SET FG <ANDB .CLR *377*>>
	<COLOR .BG .FG>
 	;<HLIGHT ,K-H-INV>
	...
	<PRINTF ,K-DIROUT-TBL>
	<COLOR .FG .BG>
	;<RT-PRINTF ,K-DIROUT-TBL> ;"if printf"
	  ...
	  <REPEAT ()
		 ...
		 <REPEAT ()
		    <COND (<EQUAL? <GETB .PTR <- .L 1>> !\ >
		     ...
		     (T
		      <SET L .N>)>
	       <COND (.SP? <PRINTT .PTR <- .L 1>>)
		     (T <PRINTT .PTR .L>)>
	       <SET N <- .N .L>>
	       <SET PTR <ZREST .PTR .L>>
	       ;"Erase rest of line with spaces."
	       <SET L <- <WINGET 3 ,K-W-XSIZE> <WINGET 3 ,K-W-XCURPOS>>>
	       <COND
		(<G? .L 0>
		 <XERASE 1>)>
		 <CRLF>)>>)
...
<ROUTINE RT-COUNT-LINES (TBL "AUX" (CNT 0))
	 <REPEAT (I (PTR .TBL))
	    <COND
	     ...
	     (T
	      <INC CNT>
	      <SET PTR <REST .PTR <+ .I 2>>>)>>
	 <RETURN .CNT>>

;<ROUTINE RT-PRINTF (TBL "AUX" I)
	 <REPEAT ()
	    <COND (<ZERO? <SET I <ZGET .TBL 0>>>
		   <RETURN>)
		  (T
		   <SET TBL <ZREST .TBL 2>>
		   <PRINTT .TBL .I>
		   <XERASE 1>
		   <COND (<NOT <EQUAL? <WINGET -3 ,K-W-XCURPOS> 1>>
			  <CRLF>)>
		   <SET TBL <ZREST .TBL .I>>)>>>
	 ...
	 <COND
	  (<SET SC <GETP .OBJ ,P?SCORE>>
	   <RT-SCORE-MSG
	    ...
	    <MOD .SC ;<SHIFT .SC %<- ,K-QUEST-SHIFT>> 32>	;"Quest"
	    .NL?>
	   <PUTP .OBJ ,P?SCORE 0>
	   <RTRUE>)>>
...
<ROUTINE V-SCORE ("OPT" (M? <>))
	 <COND
	  ...
	  >
	 <TELL "ou have" wn ,GL-SC-CHV " chivalry point">
	<COND
	...
	<COND
	 ...
	 >
	<TELL " proven yourself worthy of the rank of ">
	<COND
		(<L? ,GL-SC-QST 11>
			<TELL "serf">)
		(<L? ,GL-SC-QST 26>
...
<GET ,K-COLOR-TABLE ,GL-B-COLOR> " background. Is that what you want?|">
		<COND
			(<Y?>
				<RETURN>)>
		<COND
			(<AND <EQUAL? ,GL-MACHINE ,MACINTOSH>
				...
				<RETURN>)>
		<TELL
"|Do you want to pick again, or would you like to just keep the
current colors? (Type Y to pick again)>">
		<COND (<Y? <>> <CRLF>)
		      (T
		       <SETG GL-F-COLOR .FG>
		       <SETG GL-B-COLOR .BG>
...
<ROUTINE RT-COLOR-ALL-WINDOWS ("AUX" S)
	 <SET S 0>
	 <REPEAT ()
	    <SCREEN .S>
...
<ROUTINE RT-DO-COLOR ()
	<COND
		(<OR <NOT <BTST <LOWCORE FLAGS> 64 ;,F-COLOR>>
		     <AND <EQUAL? ,GL-MACHINE ,MACINTOSH>
			  <NOT <MAC-II?>> ;"b&w Mac">>
		 <COND
		  (<EQUAL? ,GL-B-COLOR 2>
...
<ROUTINE RT-CHECK-ADJ (DOOR)
	<COND
		...
		(<EQUAL? .DOOR ,LG-IVORY-DOOR>
			<RT-UPDATE-ADJ ,LG-IVORY-DOOR ,RM-TOW-CLEARING ,RM-CIRC-ROOM>)
		(<EQUAL? .DOOR ,LG-WOODEN-DOOR>
			<RT-UPDATE-ADJ ,LG-WOODEN-DOOR ,RM-LANDING ,RM-TOWER-ROOM>)>>

<ROUTINE RT-UPDATE-ADJ (DOOR RM1 RM2 "AUX" TMP1 TMP2)
	<COND
		(<AND <SET TMP2 <GETP .RM1 ,P?ADJACENT>>
				<SET TMP1 <INTBL? .RM2 <REST .TMP2 1> <GETB .TMP2 0> 1>>>
			<PUTB .TMP1 1 <FSET? .DOOR ,FL-OPEN>>)>
	<COND
		(<AND <SET TMP2 <GETP .RM2 ,P?ADJACENT>>
...
<ROUTINE RT-END-OF-GAME ("OPT" (WIN? <>) (RPT <>) "AUX" VAL (MSG? T) (M? <>))
	<COND (<NOT .WIN?> <UPDATE-STATUS-LINE>)>
	<COND
		(<NOT .RPT>
			<CRLF>
			<COND
...
"[Congratulations on winning the game.">
				 <COND (,GL-HINTS-OFF
					<SETG GL-HINTS-OFF <>>
					<TELL
" If you are curious about the background of Arthur, you might want to look at the Notes section of the Hints.">)> 
				 <TELL "]" CR CR>)>
			<COND
