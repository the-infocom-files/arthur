...
; "file : JOUST.ZIL"
; "auth :   $Author:   DEB  $"
; "date :     $Date:   16 May 1989 21:07:06  $"
; "revs : $Revision:   1.100  $"
; "vers : 1.0"
;"---------------------------------------------------------------------------"
; "Joust"
		...
		(<VERB? EXAMINE>
			...
			<COND
				(<NOUN-USED? ,TH-RED-LANCE ,W?LANCES>
					<TELL "They both look the same, except for the color." CR>)
				(T
...
<ROUTINE RT-TH-HORSE ("OPT" (CONTEXT <>))
	<COND
		...
		(.CONTEXT
			<RFALSE>)
		(<VERB? CLIMB-ON SIT>
		 <COND (<MC-FORM? ,K-FORM-ARTHUR>
			<COND (<IN? ,CH-PLAYER ,TH-HORSE>
			       <TELL "You are already on the horse." CR>)
			      (T
			       <MOVE ,WINNER ,TH-HORSE>
			       <TELL The+verb ,WINNER "are" " now on the horse.">
			       <COND (<IN? ,CH-BLUE-KNIGHT ,RM-EDGE-OF-WOODS>
				      <SETUP-ORPHAN-NP "take lance"
						       ,TH-RED-LANCE ,TH-GREEN-LANCE>
				      <TELL
				       " \"Jolly good,\" says the knight. \""
				       ,K-WHICH-LANCE-MSG>)
				     (T
				      <CRLF>)>
			       <SETG GL-UPDATE-WINDOW <BOR ,GL-UPDATE-WINDOW
							   ,K-UPD-DESC>>
			       <COND (<EQUAL? ,GL-WINDOW-TYPE ,K-WIN-DESC>
				      <RT-UPDATE-DESC-WINDOW>)>
			       <RTRUE>)>)
		       (T
		...
		(<VERB? ASK-ABOUT>
			<COND
				(<MC-PRSI? ,TH-RED-LANCE ,TH-GREEN-LANCE>
					<TELL
"\"They're both the same, old bean. Knight's honor and all that.\"" CR>)
...
in the country.\"" CR>)
						(T
							<TELL
"\"He's one of the best-trained jousting horses in the country.\"" CR>)>)
				(<MC-PRSI? ,CH-MERLIN>
					<TELL
...
lances.\"" CR>
		...
		(<VERB? LOOK-BEHIND>
			<TELL "It's not polite to look in other people's back yards." CR>)>>
