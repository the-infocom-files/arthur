...
; "file : GLOBAL.ZIL"
; "auth :   $Author:   DEB  $"
; "date :     $Date:   16 May 1989 21:08:54  $"
; "rev  : $Revision:   1.124  $"
; "vers : 1.0"
;"---------------------------------------------------------------------------"
; "Global objects"
	  ...
	  (T
	   <COND
	    (<AND <SET PERSON <GET-OWNER ,TH-HUMAN-BODY>>
		  <NOT <VISIBLE? .PERSON>>>
	     <NP-CANT-SEE>)
	    (<AND <EQUAL? .PERSON ,CH-CELL-GUARD>
		  <NOUN-USED? ,TH-HUMAN-BODY ,W?HEAD>
		  <VERB? ATTACK BREAK>
		  <NOT ,NOW-PRSI>>
	     <PERFORM ,PRSA ,CH-CELL-GUARD ,PRSI>)
	    (T
	     <TELL ,K-NO-REFER-MSG>
	     <COND
	      (<EQUAL? .PERSON
	     ...
	     <TELL " part of">
	     <COND
	      (<EQUAL? .PERSON ,CH-PLAYER> <TELL " your">)
	      (.PERSON
	       <TELL the .PERSON "'">
	       <COND
...
<OBJECT TH-HANDS
	...
	(ADJECTIVE OWL BADGER LEFT RIGHT)
	(OWNER CH-PLAYER)
	(GENERIC RT-GN-BODY)
	(ACTION RT-TH-HANDS)>
...
<ROUTINE RT-NO-BODY-PART-MSG (OBJ "AUX" TBL CNT)
	...
	<COND
		...
		(T
			...
			<COND
				(<AND ,NOW-PRSI ,PRSI-NP>
				 <NP-PRINT ,PRSI-NP>)
				(<AND <NOT ,NOW-PRSI> ,PRSO-NP>
				 <NP-PRINT ,PRSO-NP>)
				(ELSE <PRINTD .OBJ>)>
			<DIROUT ,D-TABLE-OFF>
...
<OBJECT TH-TIME
	...
	(SYNONYM
		...
		HOUR HOURS MATINS LAUDS PRIME TERCE SEXT NONE VESPERS COMPLINE
...
<OBJECT TH-UTHER
	...
	(FLAGS FL-NO-ARTICLE FL-VOWEL)
	(SYNONYM UTHER PENDRAGON FATHER)
	(ADJECTIVE UTHER)>
...
<CONSTANT K-ROMAN-FORT-MSG
"It's part of the crumbling fortifications left over from the days of the
Roman occupation.">
		...
		(<AND <VERB? CLIMB-UP CLIMB-ON CLIMB-OVER>
			...
			<COND
				...
				(<MC-FORM? ,K-FORM-OWL>
					<RT-DO-WALK ,P?UP>)
				(T
					<TELL The ,LG-WALL " is too high." CR>)>)
		(<VERB? LOOK-BEHIND>
