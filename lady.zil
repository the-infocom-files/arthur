...
; "file : LADY.ZIL"
; "auth :   $Author:   DEB  $"
; "date :     $Date:   18 May 1989 13:21:26  $"
; "revs : $Revision:   1.60  $"
; "vers : 1.0"
;"---------------------------------------------------------------------------"
		...
		(T
			...
			<COND
				(<FSET? ,CH-NIMUE ,FL-ASLEEP>
					...
					<TELL
"The woman's eyelids flutter briefly, and then she awakens. She">)
				(T
					<TELL "|The woman">)>
			<TELL
...
challenge King Lot. You will have to fight him upon the Field of Honor, but
...
<ROUTINE RT-CALL-NIMUE ("AUX" WIS EXP QST P)
	<COND
...
a perfectly dry path that leads south to the center of the lake." CR>
			<SETG GL-PICTURE-NUM ,K-PIC-KING>
			<COND
			 (<EQUAL? ,GL-WINDOW-TYPE ,K-WIN-PICT>
			  <RT-UPDATE-PICT-WINDOW>)>
			<SET WIS 0>
			<SET EXP 1>
			<SET QST 1>
			; "If player hasn't already earned points for cellar in ivory tower,
			...
			<COND
				(<FSET? ,RM-CELLAR ,FL-LOCKED>
					...
					<SET WIS <+ .WIS 4>>
					<SET QST <+ .QST 1>>)>
			; "If player hasn't already earned points for abandoned room in
				ivory tower, give them to him now."
			<SET P <GETP ,RM-CRACK-ROOM ,P?SCORE>>
			<COND
				(<NOT <ZERO? .P>>
					<SET WIS <+ .WIS <MOD <SHIFT .P %<- ,K-WISD-SHIFT>> 32>>>
					<SET EXP <+ .EXP <MOD <SHIFT .P %<- ,K-EXPR-SHIFT>> 32>>>
					<SET QST <+ .QST <MOD .P ;<SHIFT .P %<- ,K-QUEST-SHIFT>> 32>>>)>
			<RT-SCORE-MSG 0 .WIS .EXP .QST>
		;	<COND
				(<MC-HERE? ,RM-SHALLOWS>
					<SETG GL-UPDATE-WINDOW <BOR ,GL-UPDATE-WINDOW ,K-UPD-DESC>>
					<COND
