...
<ROUTINE RT-HOT-KEY (KEY "OPT" (BUF ,P-INBUF) (OH ,GL-PIC-HIGH))
	 <SETG GL-PIC-HIGH 0>
	 ;"Suppress graphics clear in most cases"
	<COND
		(<EQUAL? .KEY ,K-F1>
			<COND
				...
				(T
				 <COND (<==? ,GL-WINDOW-TYPE ,K-WIN-PICT>
					<SETG GL-PIC-HIGH .OH>)>
...
<ROUTINE RT-UPDATE-INVT-WINDOW ("OPT" (REF? <>) "AUX" WX WY W X Y I J K N)
	...
	<SETG GL-WIN-Y 1>
	<SETG GL-WIN-X 1>
	<RT-PUT-UP-FOO <COND (<MC-APPLE?> "Carrying:")
	...
	<COND
	 (<AND <G? ,GL-WIN-Y 1>
	  ...
	  <SETG GL-WIN-X <+ ,GL-WIN-X .W 2>>
	  <SETG GL-WIN-Y 1>)>
	<RT-PUT-UP-FOO <COND (<MC-APPLE?> "Wearing:")
...
<ROUTINE RT-REDRAW-MAP ("AUX" TBL PTR N Y X RM RM-ICON OLD-NUM)
	...
	<COND
	 ...
	 (T ;"Display background scroll."
	  <SCREEN 7>
	  ;<PICINF ,K-MAP-SCROLL-POS ,K-WIN-TBL>
	  <DISPLAY ,K-MAP-SCROLL 1 1
		   ;"<ZGET ,K-WIN-TBL 0> <ZGET ,K-WIN-TBL 1>">
	  <SCREEN 2>
	  <MOUSE-LIMIT 2>
...
<ROUTINE RT-DRAW-ROSE ("AUX" X Y (DIR 0) P D)
	...
	<COND
		(<EQUAL? ,GL-MACHINE ,MACINTOSH ,DEBUGGING-ZIP>
			<SET Y 3>)
		(T
			<SET Y 2>)>
...
<GLOBAL GL-BANNER-Y 0> ;"y where banner is displayed"
<GLOBAL GL-BANNER-X 0> ;"x where banner is displayed"
<GLOBAL GL-PIC-HIGH 0>
<GLOBAL GL-PIC-WIDE 0>

<ROUTINE RT-ROOM-PIC (N "AUX" X Y)
	 <COND (<ZERO? ,GL-OFF-Y> <RT-BANNER-OFFSET>)>
	 <COND (<G? ,GL-PIC-HIGH 0>
		<WINPOS 4 <+ ,WINDOW-2-Y ,GL-WIN-Y>
			<+ ,WINDOW-2-X ,GL-WIN-X>>
		<WINSIZE 4 ,GL-PIC-HIGH ,GL-PIC-WIDE>
		<CLEAR 4>)>
	 <PICINF .N ,K-WIN-TBL>
	 <SETG GL-PIC-HIGH <ZGET ,K-WIN-TBL 0>>
	 <SETG GL-PIC-WIDE <ZGET ,K-WIN-TBL 1>>
	 <SET Y <+ ,GL-BANNER-Y </ <- ,GL-OFF-Y <ZGET ,K-WIN-TBL 0>> 2>>>
	 <SET X <ZGET ,K-WIN-TBL 1>>
	 <SET X <+ </ <- <WINGET -3 ,K-W-XSIZE> .X> 2> 1>>
	 <COND (<EQUAL? ,GL-PIC-WIDE 195> <SET X <+ .X 1>>)>
	 <DISPLAY .N .Y .X>
	 <SETG GL-WIN-N .N>
	 <SETG GL-WIN-Y .Y>
	 <SETG GL-WIN-X .X>
...
<ROUTINE RT-BANNER-OFFSET ("AUX" X Y)
	 <COND (<PICINF ,K-PIC-BANNER ,K-WIN-TBL>
		<SET Y <GET ,K-WIN-TBL 0>>
		<SET X <GET ,K-WIN-TBL 1>>
		<COND (<PICINF ,K-PIC-BANNER-R ,K-WIN-TBL>
		       <SET Y <+ .Y <GET ,K-WIN-TBL 0>>>)>
		<SETG GL-BANNER-Y <+ </ <- <WINGET 7 ,K-W-YSIZE> .Y> 2> 1>>
		<SETG GL-BANNER-X <+ </ <- <WINGET 7 ,K-W-XSIZE> .X> 2> 1>>)>
	 <COND (<PICINF ,K-PIC-BANNER-MARGIN ,K-WIN-TBL>
		<SETG GL-OFF-Y <ZGET ,K-WIN-TBL 0>>)>>

<ROUTINE RT-BANNER-PIC ("AUX" (W 0) (H 0))
	 <COND (<ZERO? ,GL-OFF-Y> <RT-BANNER-OFFSET>)>
	 <COND (<PICINF ,K-PIC-BANNER ,K-WIN-TBL>
		<SET H <GET ,K-WIN-TBL 0>>
		<SET W <GET ,K-WIN-TBL 1>>
		<DISPLAY ,K-PIC-BANNER ,GL-BANNER-Y ,GL-BANNER-X>
		<COND (<PICINF ,K-PIC-BANNER-R ,K-WIN-TBL>
		       <DISPLAY ,K-PIC-BANNER-L
				<+ ,GL-BANNER-Y .H> ,GL-BANNER-X>
		       <DISPLAY ,K-PIC-BANNER-R
				<+ ,GL-BANNER-Y .H>
				<- <+ ,GL-BANNER-X .W>
				   <GET ,K-WIN-TBL 1>>>)>)>
	<RTRUE>>
...
<ROUTINE RT-UPDATE-PICT-WINDOW ("OPT" (REF? <>)
				"AUX" STAMP OFF ST? ;"DX DY")
	 <SCREEN 2>
	 <WINATTR 2 ,A-WRAP ,O-CLEAR>	;"Make window 2 not wrap."
	 <COND
	 ...
	 <COND
	  ...
	  (<G? ,GL-WIN-N 0>
	   <COND
	    (<NOT <EQUAL? ,GL-PICTURE-NUM ,GL-WIN-N>>
	     <COND (<AND <PICINF ,GL-PICTURE-NUM ,K-WIN-TBL>
			 <EQUAL? ,GL-PIC-HIGH <GET ,K-WIN-TBL 0>>
			 <EQUAL? ,GL-PIC-WIDE <GET ,K-WIN-TBL 1>>>
		...
		(<EQUAL? ,GL-PICTURE-NUM ,K-PIC-RAVEN-NEST>
		 <COND
		  (<OR	<IN? ,TH-RAVEN-EGG ,RM-RAVEN-NEST>
		   ...
		   ; "Stamp kludge"
		     ;	<COND
			 (<EQUAL? ,GL-MACHINE ,MACINTOSH>
			  <SET DY 0>
			  <SET DX 2>)
			 (<EQUAL? ,GL-MACHINE ,IBM>
			  <SET DY 0>
			  <SET DX 1>)>
